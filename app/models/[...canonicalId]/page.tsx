...
export default async function ModelDetailPage({ params, searchParams }: { params: Promise<{ canonicalId: string[] }>; searchParams: SearchParams }) {
  ...
  return <AppShell locale={locale} section={msg(locale, "models")} detail={model.name}>
    <div className="detail-header">
      <div className="identity"><span className="identity-mark">{initials}</span><div><h1>{model.name}</h1><p>{model.canonicalId}</p></div>{model.openWeights && <span className="tag success">Open weights</span>}{model.lifecycle.status !== "active" && <span className={`tag${model.lifecycle.status === "sunset" ? " warning" : ""}`} title={lifecycleTitle}>{lifecycleLabel(model.lifecycle.status)}</span>}</div>
