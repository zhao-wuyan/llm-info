...
type ChannelSortKey = "provider" | "usd" | "cny";
...
export default async function ModelDetailPage({ params, searchParams }: { params: Promise<{ canonicalId: string[] }>; searchParams: SearchParams }) {
  const [locale, { canonicalId }, queryParams] = await Promise.all([getLocale(), params, searchParams]);
  const id = canonicalId.map(decodeURIComponent).join("/");
  const model = modelByCanonicalId.get(id);
  if (!model) notFound();
  ...
  const sort: ChannelSortKey | null = rawSort === "provider" || rawSort === "usd" || rawSort === "cny" ? rawSort : null;
  ...
  const channels = sort && order ? stableSort(model.channels, (left, right) => {
    ...
    const currency = sort === "usd" ? "USD" : "CNY";
    return compareNullable(priceRate(left.displayPrices[currency], "textInput"), priceRate(right.displayPrices[currency], "textInput"), order) || providerNames[left.providerId].localeCompare(providerNames[right.providerId]);
  }) : model.channels;
  ...
  return <AppShell locale={locale} section={msg(locale, "models")} detail={model.name}>
    ...
    <div className="detail-grid"><div className="detail-main">
      ...
      <section className="panel"><header className="panel-header"><h2>{msg(locale, "pricingOverview")}</h2><span className="mono">{msg(locale, "priceUnit")}</span></header><div className="table-frame borderless"><table className="data-table"><thead><tr><th>{msg(locale, "currency")}</th><th>{msg(locale, "inputPrice")}</th><th>{msg(locale, "outputPrice")}</th><th>{msg(locale, "cacheReadPrice")}</th><th>{msg(locale, "cacheCreationPrice")}</th><th>{msg(locale, "source")}</th></tr></thead><tbody>{(["USD", "CNY"] as const).map((currency) => <tr key={currency}><td className="mono">{currency}</td><td><PriceValue price={model.minPrices[currency]} rate="textInput" currency={currency} locale={locale} /></td><td><PriceValue price={model.minPrices[currency]} rate="textOutput" currency={currency} locale={locale} /></td><td><PriceValue price={model.minPrices[currency]} rate="textInput_cacheRead" currency={currency} locale={locale} /></td><td><PriceValue price={model.minPrices[currency]} rate="textInput_cacheWrite" currency={currency} locale={locale} /></td><td><span className="tag">{model.minPrices[currency]?.source ?? "-"}</span></td></tr>)}</tbody></table></div></section>
      <section className="panel"><header className="panel-header"><h2>{msg(locale, "providerPreview")}</h2><span className="mono">{model.channels.length}</span></header><div className="table-frame borderless"><table className="data-table model-channel-table"><thead><tr><SortableHeader label={msg(locale, "providers")} direction={directionFor("provider")} href={sortLinkFor("provider")} locale={locale} /><SortableHeader label="USD" direction={directionFor("usd")} href={sortLinkFor("usd")} locale={locale} /><SortableHeader label="CNY" direction={directionFor("cny")} href={sortLinkFor("cny")} locale={locale} /><th>{msg(locale, "source")}</th></tr></thead><tbody>{channels.slice(0, 6).map((channel) => <tr key={channel.id}><td><Link className="entity-name" href={`/providers/${encodeURIComponent(channel.providerId)}`}><EntityText name={providerNames[channel.providerId]} id={channel.providerId} /></Link></td><td><PriceValue price={channel.displayPrices.USD} rate="textInput" currency="USD" locale={locale} /></td><td><PriceValue price={channel.displayPrices.CNY} rate="textInput" currency="CNY" locale={locale} /></td><td><div className="tag-list">{channel.sourceRefs.slice(0, 2).map((ref) => <span className="tag" key={`${ref.source}:${ref.id}`}>{ref.source}</span>)}</div></td></tr>)}</tbody></table></div></section>
