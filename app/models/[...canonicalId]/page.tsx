...
export default async function ModelDetailPage({ params, searchParams }: { params: Promise<{ canonicalId: string[] }>; searchParams: SearchParams }) {
  ...
  return <AppShell locale={locale} section={msg(locale, "models")} detail={model.name}>
    ...
    <div className="detail-grid"><div className="detail-main">
      ...
      {model.weights && <section className="panel panel-muted"><header className="panel-header"><h2>{msg(locale, "weightsEvidence")}</h2><span className="mono">{model.weights.source}</span></header><div className="panel-body"><dl className="definition-list"><dt>{msg(locale, "openWeights")}</dt><dd><a className="source-link" href={model.weights.repoUrl} target="_blank" rel="noreferrer">{model.weights.repoId} <ExternalLink size={13} /></a></dd><dt>{msg(locale, "modelLicense")}</dt><dd>{model.weights.license ? <>{model.weights.license} <small>({model.weights.licenseType === "spdx" ? msg(locale, "spdxLicense") : msg(locale, "customLicense")})</small></> : "-"}</dd><dt>{msg(locale, "downloads")}</dt><dd>{compactNumber(model.weights.downloads ?? undefined)}</dd><dt>{msg(locale, "likes")}</dt><dd>{compactNumber(model.weights.likes ?? undefined)}</dd><dt>{msg(locale, "parameters")}</dt><dd>{compactNumber(model.weights.parameters ?? undefined)}</dd>{model.weights.gated && <><dt>{msg(locale, "gatedRepo")}</dt><dd>{msg(locale, "supported")}</dd></>}<dt>{msg(locale, "observedAt")}</dt><dd>{formatDate(model.weights.lastModified ?? model.weights.observedAt)}</dd></dl></div></section>}
