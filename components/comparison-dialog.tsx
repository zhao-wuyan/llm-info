...
type ComparisonSortKey = "provider" | "input" | "output";
...
export function ComparisonDialog({ locale, canonicalId, channels, providerNames }: { locale: Locale; canonicalId: string; channels: Model[]; providerNames: Record<string, string> }) {
  ...
  const rows = useMemo(() => {
    ...
    return stableSort(matching, (left, right) => {
      ...
      const metric = sort === "input" ? "textInput" : "textOutput";
      return compareNullable(priceRate(left.displayPrices[currency], metric), priceRate(right.displayPrices[currency], metric), order) || (providerNames[left.providerId] ?? left.providerId).localeCompare(providerNames[right.providerId] ?? right.providerId);
    });
  ...
  return <>
    ...
    <dialog ref={dialog} className="modal" aria-labelledby="provider-comparison-title" onClick={(event) => { if (event.target === dialog.current) dialog.current.close(); }}>
      <div className="modal-layout">
        ...
        <div className="modal-content"><table className="data-table comparison-provider-table"><thead><tr><SortableButtonHeader label={msg(locale, "providers")} direction={directionFor("provider")} onSort={() => toggleSort("provider")} locale={locale} /><SortableButtonHeader label={msg(locale, "inputPrice")} direction={directionFor("input")} onSort={() => toggleSort("input")} locale={locale} /><SortableButtonHeader label={msg(locale, "outputPrice")} direction={directionFor("output")} onSort={() => toggleSort("output")} locale={locale} /><th>{msg(locale, "source")}</th><th>{msg(locale, "observedAt")}</th><th aria-label={msg(locale, "details")} /></tr></thead><tbody>{rows.map((channel) => <tr key={channel.id} tabIndex={0} role="link" onClick={() => router.push(`/providers/${encodeURIComponent(channel.providerId)}`)} onKeyDown={(event) => { if (event.key === "Enter") router.push(`/providers/${encodeURIComponent(channel.providerId)}`); }}><td className="entity-cell"><span className="entity-name"><EntityText name={providerNames[channel.providerId] ?? channel.providerId} id={channel.providerId} /></span></td><td><PriceValue price={channel.displayPrices[currency]} rate="textInput" currency={currency} locale={locale} /></td><td><PriceValue price={channel.displayPrices[currency]} rate="textOutput" currency={currency} locale={locale} /></td><td><span className="tag">{channel.displayPrices[currency]?.source ?? "-"}</span></td><td className="mono">{formatDate(channel.pricing.find((price) => price.id === channel.displayPrices[currency]?.priceId)?.observedAt)}</td><td><ChevronRight size={15} aria-hidden /></td></tr>)}</tbody></table></div>
